...
class Snake:
    ...
    def __init__(self, game_map, init_direc=None, init_bodies=None, init_types=None):
        ...
        self.reset(False)
        self.count_step = 0  # 从吃到食物后计数，当循环次数大于网格数强制吃食物

        #计算蛇速度
        self._start_time=time.time()
        self._interval_time=0.3
    ...
    def move(self, new_direc=None):
        ...
        if (
           ...
           # or  self.head().adj(self._direc_next) in self._bodies
        ):
            return "game_over"
        # if not self._map.has_food():
        #     for i in range(1,len(self._bodies)):
        #         if self.head()==self._bodies[i]:
        #             return "game_over"
        ...
        if self._map.point(new_head).type == PointType.FOOD:
            ...
            #     print(len(self._bodies))
            self.iseatfood=True
            self._map.rm_food(new_head)
            self.count_step=0
        else:
            self._rm_tail()
        ...
        self._steps += 1
        self.count_step+=1
        # print("count_step",self.count_step)
        # print("step",self._steps)
        if self._map.has_food():
            self.velocity_step+=1
            if (time.time()-self._start_time)>self._interval_time:
                self.velocity=self.velocity_step/self._interval_time
                self.velocity_step=1
                self._start_time=time.time()
            # print(self.velocity)
