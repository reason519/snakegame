...
from snake.util.PriorityQueue import PriorityQueue
...
class PathSolver(BaseSolver):
    ...
    def shortest_path_to_tail(self):
        return self.path_to(self.snake.tail(), "shortest")

    # def longest_path_to_tail_with_shortestpath(self,des,path):
    ...
    def shortest_path_to(self,des):
        #使用Best First Search 搜索策略
        self._reset_table()

        head = self.snake.head()
        self._table[head.x][head.y].dist = head.manhattan_dist(head,des)
        priorityQueue = PriorityQueue()
        priorityQueue.put(head,self._table[head.x][head.y].dist)

        while not priorityQueue.isEmpty():
            cur = priorityQueue.get()
            if cur == des:
                return self._build_path(head, des)
            ...
            for pos in adjs:
                if self._is_valid(pos):
                    ...
                    if adj_cell.dist == sys.maxsize:
                        adj_cell.parent = cur
                      #  adj_cell.dist = self._table[cur.x][cur.y].dist + 1
                        adj_cell.dist=pos.manhattan_dist(pos,des)
                        priorityQueue.put(pos,adj_cell.dist)
